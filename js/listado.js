...
Listado.prototype.buscarRestaurante = function(id) {
    ...
    var restauranteEncontrado = this.restaurantes.find(function(restaurant){
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    arregloUbicaciones = this.restaurantes.map(function(restaurant){
        return restaurant.ubicacion;
    })
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    var ubicacionesSinRepetir = this.eliminaElementosRepetidos(arregloUbicaciones)
...
Listado.prototype.obtenerRubros = function() {
    ...
    arreglorubro = this.restaurantes.map(function(restaurant){
        return restaurant.rubro;
    })

    var rubroSinRepetir = this.eliminaElementosRepetidos(arregloRubro);

    return rubroSinRepetir.sort();
}
...
Listado.prototype.obtenerHorarios = function() {
    ...
    //Obtener horarios de cada restaurante
    arregloHorario = this.restaurantes.map(function(restaurant){
        return restaurant.horarios;
    })

    //En este arreglo vamos a poner todos los horarios, uno por uno
    var arrayUnificadorDeHorarios = [];
    arregloHorario.forEach(function(arregloDeHorarios) {
        arregloDeHorarios.forEach(function(hora) {
            arrayUnificadorDeHorarios.push(hora)
        });
    });

    //En este arreglo vamos a poner todos los horarios pero sin repetidos
    var horariosSinRepetir = this.eliminaElementosRepetidos(arrayUnificadorDeHorarios);

    return horariosSinRepetir.sort();
}

//Elimina elementos repetidos de un arreglo
Listado.prototype.eliminaElementosRepetidos = function(arrayAFiltrar) {
    var arraySinRepetir = arrayAFiltrar.filter(function(elem, index, self) {
